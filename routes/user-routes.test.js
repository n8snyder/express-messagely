...
describe("User Routes Test", function () {
  ...
  describe("GET /users/:username", function () {
    ...
    test("wrong user", async function () {
      const token2 = jwt.sign({ username: user2.username }, SECRET_KEY);
  ...
  describe("GET users/:username/to", function () {
    ...
    test("logged out user", async function () {
      let response = await request(app).get(`/users/${user2.username}/to`);

      expect(response.statusCode).toEqual(401);
    });
    test("fail getting to messages as sender", async function () {
      let response = await request(app)
        .get(`/users/${user2.username}/to`)
        .send({ _token: token1 });
  ...
  describe("GET users/:username/from", function () {
    test("get all messages from other users", async function () {
      let response = await request(app)
    ...
    test("fail getting from messages as recipient", async function () {
      const token2 = jwt.sign({ username: user2.username }, SECRET_KEY);
      let response = await request(app)
        .get(`/users/${user1.username}/from`)
        .send({ _token: token2 });
